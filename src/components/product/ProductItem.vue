<template>
  <n-card hoverable embedded class="w-full max-w-sm mx-auto rounded-2xl shadow-md">
    ...
    <div class="mt-4 space-y-2">
      ...
      <div class="mt-3">
        <div v-if="!cartItem" class="flex justify-start">
          <n-button @click="addToCart" type="primary" ghost>Add to Cart</n-button>
        </div>
        ...
        <div v-else class="flex items-center gap-2">
          ...
          <n-button @click="addToCart" type="primary" ghost>+</n-button>
        </div>
      </div>
    </div>
  </n-card>
